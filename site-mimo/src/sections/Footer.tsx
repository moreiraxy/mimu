...
const EMPRESA = [
  ...
  ["(11) 92092-4833", "https://wa.me/5511920924833"],
] as const;

/**
 * Só as redes onde a Mimu existe de verdade.
 *
 * Saíram LinkedIn, Facebook e X: vieram do template e apontavam para a home
 * de cada rede, não para um perfil. Um link que leva a lugar nenhum passa
 * menos confiança do que a ausência dele.
 *
 * Os ícones agora são SVG inline em vez dos PNGs do template — desenham
 * nítido em qualquer tela, herdam a cor por `currentColor` (não precisam
 * mais do truque de `invert` para aparecer no rodapé escuro) e não dependem
 * de arquivo de imagem para carregar.
 */
const SOCIAIS = [
  {
    rotulo: "Siga a Mimu no Instagram",
    href: "https://www.instagram.com/mimu.ia",
    caminho:
  ...
  {
    rotulo: "Fale com a Mimu no WhatsApp",
    // wa.me pede o número no formato internacional, só dígitos:
    // 55 (Brasil) + 11 (DDD) + o número.
    href: "https://wa.me/5511920924833",
    caminho:
  ...
  return (
    <div className="flex flex-col items-start gap-5">
      ...
      <div className="flex items-center gap-3">
        {SOCIAIS.map(({ rotulo, href, caminho }) => (
          <a
            key={rotulo}
            href={href}
            target="_blank"
            ...
            aria-label={rotulo}
            className="flex size-[22px] shrink-0 items-center justify-center text-cream opacity-60 transition-opacity hover:opacity-100"
          >
            <svg
              viewBox="0 0 24 24"
              fill="currentColor"
