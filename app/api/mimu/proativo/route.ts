...
export async function POST() {
  ...
  if (candidatos.length > 0) {
    ...
    // Push notification real (background) — silenciosa por design, nunca
    // deve travar a resposta se a usuária não tiver inscrição ou chaves
    // VAPID não estiverem configuradas.
    await Promise.all(
      novos.map((alerta) => {
